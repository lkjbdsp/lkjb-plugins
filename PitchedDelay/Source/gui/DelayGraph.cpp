...
void DelayGraph::paint (Graphics& g)
{
	...
	for (int i=0; i<numDelays; ++i)
	{
		...
		if (currentSelected == i && delayOffset > 0)
		{
			...
			const float ys = float(h - (h-16) * (preVolume+60)/60);
			
			Path p;
			p.startNewSubPath(xoffs, ys);
		...
		if (enabled)
		{
			...
			{
				volume += volChange;
				const float ys = float(h - (h-16) * (volume+60)/60);

				g.drawVerticalLine(int (fx+xoffset), ys, h-1);
			}
		}
	}
    //[/UserPaint]
	...
	if (dragging)
	{
		...
		if (tab != 0)
		{
			if (! tab->tbEnable->getToggleState())
				tab->tbEnable->setToggleState(true, sendNotificationSync);
...
void DelayGraph::mouseUp (const MouseEvent& e)
{
	...
	if (e.mods.isRightButtonDown())
	{
		...
		if (tabIndex >= 0)
		{
			...
			tab->cbSync->setSelectedId(newSync, sendNotificationSync);
		}
	}
	else if (e.getNumberOfClicks() > 1)
	{
		...
		if (tabIndex >= 0)
		{
			...
			tab->tbEnable->setToggleState(false, sendNotificationSync);
		}
