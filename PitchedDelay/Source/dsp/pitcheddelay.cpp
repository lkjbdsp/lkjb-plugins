...
void PitchedDelay::setDelay(double time, bool prePitch)
{
	...
	const int latencySamples = pitcher.getLatency();

	DBG("setDelay("+String(time*1000, 1)+" ms, "+String(prePitch?"prePitch)":"postPitch)") +" pitcherlatency="+String(latencySamples));

	const int delaySamplesPrePitch = jlimit(0, delayL.getDataLength(), int(time*sampleRate));
	const int delaySamples = jmax(0, preDelayPitch ? delaySamplesPrePitch : int(delayRange.clipValue(time)*sampleRate) - latencySamples);
...
{
	const int latencySamples = pitcher.getLatency();
	return preDelayPitch ? delayL.getCurrentDelay() : delayL.getCurrentDelay() + latencySamples/sampleRate;
}
...
void PitchedDelay::updateLatency(int latencySamples)
{
	latencyCompensation.setLength(latencySamples);
	unpitchedDelay.setLength(latencySamples);

	const double minDelay = (latencySamples + 10) / sampleRate;
	const double maxDelay = (delayL.getDataLength() + latencySamples - 10) / sampleRate;
