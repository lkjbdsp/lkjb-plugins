...
PitchedDelayTab::PitchedDelayTab (PitchedDelayAudioProcessor* processor, int delayindex)
    : filter(processor),
      ...
      sPreVolume (0)
{
	addAndMakeVisible (sDelay = new Slider ("DelaySlider"));
	sDelay->setTooltip ("Delay time");
	...
	addAndMakeVisible (cbSync = new ComboBox ("SyncComboBox"));
	cbSync->setTooltip ("Quantisation");
	cbSync->setEditableText (false);
	cbSync->setJustificationType (Justification::centredLeft);
	cbSync->setTextWhenNothingSelected ("");
	cbSync->setTextWhenNoChoicesAvailable ("(no choices)");
	cbSync->addItem ("seconds", 1);
	cbSync->addItem ("1/2", 2);
	cbSync->addItem ("1/2T", 3);
	cbSync->addItem ("1/4", 4);
	cbSync->addItem ("1/4T", 5);
	cbSync->addItem ("1/8", 6);
	cbSync->addItem ("1/8T", 7);
	cbSync->addItem ("1/16", 8);
	cbSync->addItem ("1/16T", 9);
	cbSync->addItem ("1/32", 10);
	cbSync->addListener (this);

	addAndMakeVisible (sPitch = new Slider ("PitchSlider"));
	sPitch->setTooltip ("Pitch-Shift");
	...
	addAndMakeVisible (tbPostPitch = new ToggleButton ("PostPitchButton"));
	tbPostPitch->setTooltip ("When checked, pitch is applied only before feedback loop. Otherwise the feedback will be pitched to. Pitching inside the feedback results in a minimum delay time.");
	tbPostPitch->setButtonText ("Pre-Feedback");
	tbPostPitch->addListener (this);

	addAndMakeVisible (sFeedback = new Slider ("FeedbackSlider"));
	sFeedback->setTooltip ("Delay feedback");
	...
	addAndMakeVisible (sFreq = new Slider ("FrequencySlider"));
	sFreq->setTooltip ("Filter in feedback loop frequency");
	...
	addAndMakeVisible (tbSemitones = new ToggleButton ("SemitonesButton"));
	tbSemitones->setTooltip ("Pitch only in semitones");
	tbSemitones->setButtonText ("Semitones");
	tbSemitones->addListener (this);

	addAndMakeVisible (sQfactor = new Slider ("QSlider"));
	sQfactor->setTooltip ("Filter in feedback loop Q-factor");
	...
	addAndMakeVisible (cbFilter = new ComboBox ("FilterTypeComboBox"));
	cbFilter->setTooltip ("Filter Type. Maximum filter gain is limited to 0 dB to avoid feedback oscillation.");
	cbFilter->setEditableText (false);
	cbFilter->setJustificationType (Justification::centredLeft);
	cbFilter->setTextWhenNothingSelected ("");
	cbFilter->setTextWhenNoChoicesAvailable ("(no choices)");
	cbFilter->addItem ("Off", 1);
	cbFilter->addItem ("Lowpass", 2);
	cbFilter->addItem ("Highpass", 3);
	cbFilter->addItem ("Lowshelf", 4);
	cbFilter->addItem ("Highshelf", 5);
	cbFilter->addItem ("Bell", 6);
	cbFilter->addItem ("Bandpass", 7);
	cbFilter->addItem ("Notch", 8);
	...
	addAndMakeVisible (sGain = new Slider ("GainSlider"));
	sGain->setTooltip ("Filter in feedback loop gain (where available)");
	...
	addAndMakeVisible (tbEnable = new ToggleButton ("EnabledButton"));
	tbEnable->setButtonText ("Enabled");
	tbEnable->addListener (this);

	addAndMakeVisible (sVolume = new Slider ("VolumeSlider"));
	sVolume->setTooltip ("Delay tab volume");
	...
	addAndMakeVisible (cbPitch = new ComboBox ("new combo box"));
	cbPitch->setTooltip ("Pitch shfiting algorithm. Off Disables pitching and saves resources.");
	cbPitch->setEditableText (false);
	cbPitch->setJustificationType (Justification::centredLeft);
	cbPitch->setTextWhenNothingSelected ("");
	cbPitch->setTextWhenNoChoicesAvailable ("(no choices)");
	cbPitch->addListener (this);

	addAndMakeVisible (tbMono = new ToggleButton ("MonoTogglebutton"));
	tbMono->setTooltip ("Mono (mid) delay");
	tbMono->setButtonText ("Mono");
	tbMono->setRadioGroupId (5000);
	tbMono->addListener (this);
	tbMono->setToggleState (true, dontSendNotification);

	addAndMakeVisible (tbStereo = new ToggleButton ("StereoTogglebutton"));
	tbStereo->setTooltip ("Stereo delay");
	tbStereo->setButtonText ("Stereo");
	tbStereo->setRadioGroupId (5000);
	tbStereo->addListener (this);

	addAndMakeVisible (tbPingpong = new ToggleButton ("PingpongTogglebutton"));
	tbPingpong->setTooltip ("Stereo pingpong delay");
	tbPingpong->setButtonText ("Pingpong");
	tbPingpong->setRadioGroupId (5000);
	tbPingpong->addListener (this);

	addAndMakeVisible (lPan = new Label ("Pan/Balance Label",
									  L"Pan\n"));
	...
	addAndMakeVisible (sPan = new Slider ("pan slider"));
	...
	addAndMakeVisible (sPreDelay = new Slider ("DelaySlider"));
	sPreDelay->setTooltip ("Delay time");
	...
	addAndMakeVisible (sPreVolume = new Slider ("new slider"));
	...
	cbFilter->setSelectedId(1, dontSendNotification);
	cbSync->setSelectedId(1, dontSendNotification);
...
void PitchedDelayTab::setDelayRange(bool sendMessage)
{
	...
	filter->getPlayHead()->getCurrentPosition(info);
...
void PitchedDelayTab::setParam(int index, double value)
{
	switch (index)
	{
		...
		case DelayTabDsp::kSync:
			cbSync->setSelectedId((int) value+1, dontSendNotification);
			break;
		...
		case DelayTabDsp::kPitchType:
			cbPitch->setSelectedId((int) value + 1, dontSendNotification);
		...
		case DelayTabDsp::kFilterType:
			cbFilter->setSelectedId((int) value + 1, dontSendNotification);
